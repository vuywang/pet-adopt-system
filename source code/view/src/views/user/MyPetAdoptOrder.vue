  ...
  <div class="container">
      ...
      <el-table :data="apiResult.data">
        ...
        <el-table-column prop="status" width="120" label="订单状态">
          <template #default="scope">
            ...
            <el-tag tsize="mini" ype="success" v-else="scope.row.status === 4">已完成</el-tag>
          </template>
        </el-table-column>
              ...
              <div class="operate-buttons">
                <el-dropdown trigger="click" placement="bottom-end">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-if="scope.row.status === 3" @click.native="againReply(scope.row)"
                      icon="el-icon-c-scale-to-original">重新提交证明材料</el-dropdown-item>
    ...
    <el-drawer :before-close="beforeClose" size="45%" title="订单详情" :visible.sync="drawer" :direction="direction">
      ...
      <div style="margin: 10px 20px;">
        ...
        <!-- 订单完成 -->
        <div style="margin-block: 10px;" v-if="petAdoptOrder.status === 2">
          <el-button type="primary" @click="completeOrders(petAdoptOrder.id)">完成订单</el-button>
        </div>
      </div>
    ...
    <!-- 删除确认弹窗 -->
    <el-dialog title="证明材料补充提交" :show-close="false" :visible.sync="dialogAgainVisible" width="40%">
      <div>
        <Editor height="300px" :receiveContent="detail" @on-listener="onListener"
          api="http://localhost:21090/api/v1.0/pet-adopt-api/file/upload" />
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="beforeClose">取消</el-button>
        <el-button size="mini" type="primary" @click="confirmAgainReply">确定</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    againReply(data) {
      this.dialogAgainVisible = true;
      this.detail = data.detail;
      this.apiParam = { ...data };
    },
    async completeOrders(id) {
      try {
        const petAdoptOrder = {
          id: id,
          status: 4 // 标识为已完成状态
        };
        await this.$axios.put('/pet-adopt-order/update', petAdoptOrder);
        this.drawer = false; // 关闭操作抽屉
    ...
    async confirmAgainReply() {
      try {
        const petAdoptOrder = {
          id: this.apiParam.id,
          detail: this.detail
        };
        const { message } = await this.$axios.put('/pet-adopt-order/againReply', petAdoptOrder);
        this.$message.success(message);
        this.dialogAgainVisible = false;
        this.fetchFreshData();
      } catch (error) {
        this.$message.warning(error.message);
      }
    ...
    async fetchFreshData() {
      try {
        const { data, total } = await this.$axios.post('/pet-adopt-order/queryUser', this.petAdoptOrderQueryDto);
