    ...
    <div class="post-container">
        <div class="nav">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>宠物经验帖子修改</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="content">
            <div class="left">
                ...
                <div @click="toPetPostUpdate" class="publish-pet-post">
                    立即修改
                </div>
            </div>
            <div class="right">
...
export default {
    ...
    created() {
        const queryDto = this.$router.currentRoute.query;
        this.fetchPetPostDetail(queryDto.id);
        this.fetchPetType();
    },
    methods: {
        async toPetPostUpdate() {
            try {
                this.petPost.cover = this.cover;
                const { message } = await this.$axios.put(`/pet-post/update`, this.petPost);
                this.$notify({
                    title: '修改宠物经验帖子',
                    type: 'success',
                    message: message,
        ...
        // 查询宠物经验帖子信息
        async fetchPetPostDetail(id) {
            try {
                const { data } = await this.$axios.get(`/pet-post/getById/${id}`);
                this.petPost = data;
                this.text = data.content; // 将查回来的详情数据，送至富文本里面处理
                this.cover = data.cover; // 封面设置
            } catch (error) {
